...
Handlebars.registerHelper("template", function(context, options) {
	...
	let compiledTemplate = template({ main: context.fn(), ...context.data.root });
...
export default class Transcoder {
	...
	constructor(asset) {
		if(asset instanceof Asset){
			this.asset = asset;
			this.metadata = this.getMetadata(this.asset.source);
		}else{
	...
	async getMetadata(path) {
		...
		try{
			...
			data = (await import(meta_file.full)).default;
		...
		try{
			...
			data = (await import(meta_file_index.full)).default;
...
export class ImageTranscoder extends Transcoder {
	...
	async generateThumb(thumb_file){
		this.log('generateThumbnail', thumb_file);
...
export class TextTranscoder extends Transcoder {
	...
	async transcode(){
		...
		const compiledTemplate = template({...this.metadata, transcoder: this})
